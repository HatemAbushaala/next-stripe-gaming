...
export default function Home() {
  ...
  const handleCheckout = async (e) => { 
    ...
    const order_response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/checkout`,{
      "email": email,
      "items": [
          {
              "quantity": 1,
    ...
    const payment_session_response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/create-payment-session`,{
      medusa_cart_id:_cart_id
    })

    const _client_secret = payment_session_response.data.cart.payment_session.data.client_secret
